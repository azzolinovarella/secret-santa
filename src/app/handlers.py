...
from src.app.utils import validate_name, format_name, validate_phone, format_phone
...
def handle_participants_dict_form(num_participants: int) -> bool:
    ...
    for i in range(num_participants):
        ...
        participant_upper_size = st.session_state.get(
            f"draft.participants.{i}.upper_size"
        )
        participant_bottom_size = st.session_state.get(
            f"draft.participants.{i}.bottom_size"
        )
        participant_shoe_size = st.session_state.get(
            f"draft.participants.{i}.shoe_size"
        )
        ...
        if not validate_phone(participant_phone):
            popup_error(f"Telefone inválido no participante {i+1}")
            all_valid = False

        if participant_upper_size.strip() == "":
            popup_error(f"Tamanho de camisa inválido do participante {i+1}")
            all_valid = False

        if participant_bottom_size.strip() == "":
            popup_error(f"Tamanho de calça inválido do participante {i+1}")
            all_valid = False

        if participant_shoe_size.strip() == "":
            popup_error(f"Tamanho de tênis inválido do participante {i+1}")
            all_valid = False
    ...
    for i in range(num_participants):
        name = format_name(st.session_state.get(f"draft.participants.{i}.name"))
        phone = format_phone(st.session_state.get(f"draft.participants.{i}.phone"))
