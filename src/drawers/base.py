...
class BaseDrawer(ABC):
    ...
    def _validate_restrictions(
        ...
        if missing:
            raise InvalidRestrictionsException(
                f"Participantes sem restrições definidas: {', '.join(missing)}"
            )
        ...
        for p in participants:
            ...
            if p not in r:
                raise InvalidRestrictionsException(
                    f"'{p}' deve estar em sua própria lista de restrições."
                )

            invalid = r - participants_set
            if invalid:
                raise InvalidRestrictionsException(
                    f"Restrições inválidas para '{p}': {', '.join(invalid)} não existe(m) nos participantes."
                )

            if participants_set - r == set():
                raise InvalidRestrictionsException(
                    f"'{p}' não pode tirar ninguém (restrições impossíveis)."
                )

    @abstractmethod
    def _draw(
        self, participants: List[str], restrictions: Dict[str, Set[str]]
    ) -> Dict[str, str]: ...  # Deve ser implementado pelas filhas
